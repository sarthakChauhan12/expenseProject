class Income {
  ...
  mapBudgetTable() {
    this.budgetCategories.forEach((item) => {
      ...
      tr.innerHTML = `<td>${item.category}</td><td>${item.budget}</td>
                          ...
                          <td><button id="budUpdBtn${item.id}" class="upd-btn">update</button></td>
                          `;
  ...
  removeBudgetRow(e) {
    ...
    else if (btnId.includes("budUpdBtn")){
      const id = btnId.split("budUpdBtn")[1];
      const budRowId = `bRow${id}`;
      const budRow = document.getElementById(budRowId);

      const cells = budRow.getElementsByTagName('td');
      const name = cells[0].textContent;
      const value = parseInt(cells[1].textContent, 10);

      document.getElementById('budgetCategoryInput').value = name;
